...
#define CONSOLE_BAUD 9600       /* Baud rate for the console port */
//#define CONSOLE_BAUD 115200       /* Baud rate for the console port */
...
init_memory_params(void)
{
 ...
 //   GMR = 0x07840180;
	...
	/* CS1/ map SROMs at 0x02000000-0207FFFF (512KB) */
...
init_pll(void)
{
   ...
   // } else {
    ...
    //}
	    PLLCRbits.MF = (FCY_33_33MHZ / (FCY_XTAL*2));
		PLLCRbits.STSIM = 1;
		CLKOCR = 0;
}
...
init_others(void)
{
     ...
     */
    SDCR = 0x0740;              /*MODULE_BASE + 51E    Recommended by User Manual */

    CICR = 0x390001;            /*MODULE_BASE + 540    SCC priorities and spreading */
    ...
    CR = 0x8001;                /*MODULE_BASE + 5CO    Reset the CP */

    /* Disable and clear any CP interrupts that might be present */
    CIMR = 0; //MODULE_BASE + 548
    CIPR = 0xFFFFFFFF; //MODULE_BASE + 544
    CISR = 0xFFFFFFFF; //MODULE_BASE + 54C
}
...
init_console_uart(void)
{
   ...
   // if (SYSOPTbits.SPEED == 0) {
	...
	//BRGC4bits.CD = (FCY_33_33MHZ / (CONSOLE_BAUD * 16));
	BRGC4bits.CD = 17; //115200
/////////////////////////////////////////////////////////////////////////////////////////
    BRGC4bits.ATB = 0;
    BRGC4bits.EN = 1;
...
main(void)
{
	...
	//PCDAT = 0x0FBFF; // 566 D8 LED ON	
	//PCDAT = 0x0FEFF; // 566 Red LED ON
	//PCDAT = 0x0FFFF; // 566 Red LED OFF
	
    //for (for_delay = 30000; for_delay > 0; for_delay--);
	...
	for (;;) {		
        switch (state) {
            ...
            case STATE_REINIT_RX_BD:
				...
				PCDAT = 0x0FAFF; // 566 Red LED ON and D8 LED ON
				//PCDAT = 0x0FFFF; // 566 Red LED OFF  and D8 LED OFF
            ...
            default:
                ...
                if (state != STATE_DEFAULT) {
					...
					PCDAT = 0x0FEFF; // 566 Red LED ON and D8 LED OFF
					//PCDAT = 0x0FFFF; // 566 Red LED OFF
                }
